...
export async function getLiveMonitorsController(request: FastifyRequest<{ Params: { workspaceId: string } }>, response: FastifyReply) {
    try {
        const workspaceId = Number(request.params.workspaceId)
        ...
        if (monitors.length === 0) return response.send({ data: {} })

        const pipeline = redis.pipeline();
        monitors.forEach((m) => pipeline.get(`monitor:${m.id}:live`))

        const results = await pipeline.exec()

        const liveState: Record<number, any> = {}

        results?.forEach(([error, stringifiedData], index) => {
            if (!error && stringifiedData) {
                liveState[monitors[index].id] = JSON.parse(stringifiedData as string)
            }
        })

        return response.status(200).send({ data: liveState })
    }
    catch (error) {
        console.error("[LIVE_STATE] Cache read failure:", error)
        return response.status(500).send({ message: "Failed to retrieve live telemetry" })
    }
} 
